...
import { apiLogin, apiRegister, apiLogout, apiFetchPets, apiCreatePet, apiUpdatePet, apiDeletePet, apiFetchAppts, apiCreateAppt, apiUpdateAppt, apiDeleteAppt, apiAdminUsers, apiAdminLogs, apiAdminSuspicious, apiResolveSuspicious, apiAdminPermissions, apiRefreshToken, clearToken, getToken, apiForgotPassword, apiResetPassword, apiAiStatus, apiAiAnalyzeUser, apiAiAnalyzeAll } from './api';
...
/* ═══════════════ NAV ═══════════════ */
function Nav({pg,go,user,role,onLogout,onChat}){
  ...
  const links=user?[
    ...
    ...(role==="admin"?[{id:"admin",l:"Admin",i:ic.shield}]:[]),
  ]:[];
  const roleBadge=role==="admin"?{label:"Admin",color:"#DC2626",bg:"#FEE2E2"}:role==="vet"?{label:"Medic",color:"#059669",bg:"#DCFCE7"}:{label:"Proprietar",color:T,bg:"#CCFBF1"};
  ...
  return(
    <nav style={{background:"#fff",borderBottom:`1px solid ${BD}`,position:"sticky",top:0,zIndex:1000,backdropFilter:"blur(10px)",backgroundColor:"rgba(255,255,255,.92)"}}>
      <div style={{maxWidth:"100%",margin:"0 auto",padding:"0 40px",display:"flex",alignItems:"center",justifyContent:"space-between",height:60}}>
        ...
        <div className="nl" style={{display:"flex",alignItems:"center",gap:2}}>
          ...
          {user&&links.map(l=><button key={l.id} className="b bg" onClick={()=>go(l.id)} style={{color:pg===l.id?(role==="admin"&&l.id==="admin"?"#DC2626":T):TX2,background:pg===l.id?`${role==="admin"&&l.id==="admin"?"#DC2626":T}10`:"transparent",borderRadius:10,fontWeight:pg===l.id?800:600}}><Ic d={l.i} s={14}/>{l.l}</button>)}
          {user&&<button className="b bg" title="Chat" onClick={onChat} style={{color:"#8B5CF6",background:"#F5F3FF",borderRadius:10,fontWeight:700}}><Ic d={ic.user} s={14}/>Chat</button>}
...
function Landing({go}){
  return(
    <div>
      <section style={{background:`linear-gradient(160deg,#F0FDFA 0%,#CCFBF1 50%,#F0FDFA 100%)`,padding:"70px 40px",overflow:"hidden"}}>
        <div className="hw fu" style={{maxWidth:"100%",margin:"0 auto",display:"flex",alignItems:"center",gap:50}}>
          <div style={{flex:1}}>
      ...
      <section style={{padding:"70px 40px",maxWidth:"100%",margin:"0 auto"}}>
      ...
      <section className="fu" style={{padding:"50px 40px",margin:"0 40px 50px",borderRadius:20,background:`linear-gradient(135deg,${T},${TD})`,textAlign:"center"}}>
...
function Auth({type,go,onLogin,toast}){
  ...
  const submit=async()=>{
    ...
    try{
      if(reg){
        await apiRegister({name:f.name,email:f.email,phone:f.phone,password:f.password,role:f.role});
        toast("Cont creat! Conectează-te.","success");go("login");
      }else{
        const u=await apiLogin(f.email,f.password);
        onLogin(u);toast(`Bine ai venit, ${u.name}!`,"success");
      }
    }catch(err){toast(err.message||"Eroare la autentificare","error");}
  };
    ...
    <div style={{minHeight:"calc(100vh - 60px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:`linear-gradient(160deg,#F0FDFA,${BG})`}}>
      <div className="c pop" style={{padding:36,maxWidth:420,width:"100%"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          ...
          {!reg&&<><p style={{textAlign:"center",fontSize:12,color:TX2,marginTop:2}}><span style={{color:T,cursor:"pointer",fontWeight:700}} onClick={()=>go("forgot")}>Ai uitat parola?</span></p>
          <div style={{textAlign:"center",fontSize:11,color:TX2,background:"#f8fafc",padding:"8px 10px",borderRadius:10,lineHeight:1.6}}>
            Admin: <b>admin@petvet.ro</b> / <b>admin123</b><br/>
...
function ForgotPassword({go,toast}){
  ...
  const submit=async()=>{
    if(!email||!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){setErr("Email invalid");return;}
    setErr("");
    try{
      const d=await apiForgotPassword(email);
      setSent(true);
      if(d.resetToken)setToken(d.resetToken);
      toast("Token de resetare generat!","success");
    }catch(e){toast(e.message||"Eroare","error");}
  };
    ...
    <div style={{minHeight:"calc(100vh - 60px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:`linear-gradient(160deg,#F0FDFA,${BG})`}}>
      <div className="c pop" style={{padding:36,maxWidth:420,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <h2 style={{fontFamily:"'Lora',serif",fontSize:22,marginBottom:4}}>Recuperare parolă</h2>
          <p style={{color:TX2,fontSize:13}}>Introdu emailul contului tău</p>
        </div>
        {!sent?(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div className={`ig ${err?"ie":""}`}><label>Email</label><input type="email" placeholder="email@exemplu.ro" value={email} onChange={e=>setEmail(e.target.value)}/>{err&&<span className="er">{err}</span>}</div>
            <button className="b bp" style={{width:"100%",justifyContent:"center",padding:11}} onClick={submit}>Trimite token reset</button>
            <p style={{textAlign:"center",fontSize:13,color:TX2}}><span style={{color:T,cursor:"pointer",fontWeight:700}} onClick={()=>go("login")}>← Înapoi la login</span></p>
          </div>
        ):(
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:"#F0FDF4",border:"1.5px solid #86EFAC",borderRadius:12,padding:14,fontSize:13,color:"#166534",lineHeight:1.6}}>
              Token generat! În producție ar fi trimis pe email.<br/>
              {token&&<><b>Token demo:</b> <code style={{fontSize:11,wordBreak:"break-all"}}>{token}</code></>}
            </div>
            <button className="b bp" style={{width:"100%",justifyContent:"center"}} onClick={()=>go("reset")}>Resetează parola →</button>
          </div>
        )}
      </div>
    </div>
...
/* ═══════════════ RESET PASSWORD ═══════════════ */
function ResetPassword({go,onLogin,toast}){
  const [f,setF]=useState({token:"",password:"",confirm:""});
  const [e,setE]=useState({});
  const upd=(k,v)=>{setF(p=>({...p,[k]:v}));if(e[k])setE(p=>{const n={...p};delete n[k];return n})};

  const submit=async()=>{
    const errs={};
    if(!f.token||f.token.length<10)errs.token="Token invalid";
    if(!f.password||f.password.length<6)errs.password="Min 6 caractere";
    if(f.password!==f.confirm)errs.confirm="Nu coincid";
    setE(errs);if(Object.keys(errs).length)return;
    try{
      const d=await apiResetPassword(f.token,f.password);
      toast("Parola resetată! Ești acum autentificat.","success");
      onLogin(d.user);
    }catch(err){toast(err.message||"Token invalid","error");}
  };
  ...
  return(
    <div style={{minHeight:"calc(100vh - 60px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,background:`linear-gradient(160deg,#F0FDFA,${BG})`}}>
      <div className="c pop" style={{padding:36,maxWidth:420,width:"100%"}}>
        <div style={{textAlign:"center",marginBottom:24}}>
          <div style={{fontSize:46,marginBottom:8}}>🔒</div>
          <h2 style={{fontFamily:"'Lora',serif",fontSize:22,marginBottom:4}}>Resetare parolă</h2>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          <div className={`ig ${e.token?"ie":""}`}><label>Token de resetare</label><input placeholder="Lipește token-ul primit" value={f.token} onChange={ev=>upd("token",ev.target.value)}/>{e.token&&<span className="er">{e.token}</span>}</div>
          <div className={`ig ${e.password?"ie":""}`}><label>Parolă nouă</label><input type="password" placeholder="Min 6 caractere" value={f.password} onChange={ev=>upd("password",ev.target.value)}/>{e.password&&<span className="er">{e.password}</span>}</div>
          <div className={`ig ${e.confirm?"ie":""}`}><label>Confirmă parola</label><input type="password" placeholder="Repetă parola" value={f.confirm} onChange={ev=>upd("confirm",ev.target.value)}/>{e.confirm&&<span className="er">{e.confirm}</span>}</div>
          <button className="b bp" style={{width:"100%",justifyContent:"center",padding:11}} onClick={submit}>Setează parola nouă</button>
          <p style={{textAlign:"center",fontSize:13,color:TX2}}><span style={{color:T,cursor:"pointer",fontWeight:700}} onClick={()=>go("forgot")}>← Înapoi</span></p>
        </div>
      </div>
    </div>
  );
...
function Dash({pets,appts,go,TK,role,svcs}){
  ...
  return(
    <div style={{maxWidth:"100%",margin:"0 auto",padding:"22px 40px 50px"}}>
...
/* ═══════════════ PETS PAGE ═══════════════ */
function PetsPage({pets,setPets,appts,toast,TK,role,user}){
  ...
  const add=async f=>{try{const np=await apiCreatePet(f,user?.id);setPets(p=>[...p,np]);setMod(null);toast(`${np.name} adăugat!`,"success");TK.track("add_pet",{name:np.name})}catch(e){toast(e.message,"error")}};
  const upd=async f=>{try{const up=await apiUpdatePet(sel.id,f);setPets(p=>p.map(pet=>pet.id===sel.id?up:pet));setMod(null);setSel(null);toast("Salvat!","success");TK.track("update_pet",{name:f.name})}catch(e){toast(e.message,"error")}};
  const rm=async pet=>{try{await apiDeletePet(pet.id);setPets(p=>p.filter(x=>x.id!==pet.id));setDel(null);toast(`${pet.name} șters.`,"info");TK.track("delete_pet",{name:pet.name})}catch(e){toast(e.message,"error")}};
  return(
    <div style={{maxWidth:"100%",margin:"0 auto",padding:"22px 40px 50px"}}>
...
function ApptsPage({appts,setAppts,pets,toast,TK,svcs,role}){
  ...
  const add=async f=>{try{const na=await apiCreateAppt(f);setAppts(p=>[...p,na]);setMod(null);toast("Programare creată!","success");TK.track("add_appt",{service:na.service})}catch(e){toast(e.message,"error")}};
  const upd=async f=>{try{const ua=await apiUpdateAppt(sel.id,f);setAppts(p=>p.map(a=>a.id===sel.id?ua:a));setMod(null);setSel(null);toast("Actualizat!","success");TK.track("update_appt")}catch(e){toast(e.message,"error")}};
  const rm=async a=>{try{await apiDeleteAppt(a.id);setAppts(p=>p.filter(x=>x.id!==a.id));setDel(null);toast("Șters.","info");TK.track("delete_appt")}catch(e){toast(e.message,"error")}};
...
function StatsPage({pets,appts,setAppts,toast,TK,svcs}){
  ...
  const addA=async f=>{try{const na=await apiCreateAppt(f);setAppts(p=>[...p,na]);setMod(null);toast("Adăugat!","success")}catch(e){toast(e.message,"error")}};
  const updA=async f=>{try{const ua=await apiUpdateAppt(sel.id,f);setAppts(p=>p.map(a=>a.id===sel.id?ua:a));setMod(null);setSel(null);toast("Salvat!","success")}catch(e){toast(e.message,"error")}};
  const delA=async a=>{try{await apiDeleteAppt(a.id);setAppts(p=>p.filter(x=>x.id!==a.id));setDel(null);toast("Șters.","info")}catch(e){toast(e.message,"error")}};
...
function Profile({user,TK,role,onResetSvcs}){
  ...
  return(
    <div style={{maxWidth:1000,margin:"0 auto",padding:"22px 40px 50px"}}>
...
function AdminPanel(){
  ...
  const load=async(t)=>{
    ...
    try{
      if(t==="users")setUsers(await apiAdminUsers());
      else if(t==="logs")setLogs(await apiAdminLogs());
      else if(t==="suspicious")setSusp(await apiAdminSuspicious());
      else if(t==="permissions")setPerms(await apiAdminPermissions());
      else if(t==="ai"){const s=await apiAiStatus();setAiStatus(s);}
    }catch{}
    setLoading(false);
  };
  ...
  const resolve=async(userId)=>{
    try{await apiResolveSuspicious(userId);setSusp(p=>p.map(s=>s.user.id===userId?{...s,resolved:true}:s))}catch{}
  };

  const analyzeUser=async(userId)=>{
    setAiAnalyzing(userId);
    try{
      const r=await apiAiAnalyzeUser(userId);
      setAiResults(p=>[{userId,name:users.find(u=>u.id===userId)?.name||`#${userId}`,...r,ts:new Date()},...p.filter(x=>x.userId!==userId)]);
    }catch(e){setAiResults(p=>[{userId,name:`#${userId}`,analysis:`Eroare: ${e.message}`,source:"error",ts:new Date()},...p.filter(x=>x.userId!==userId)]);}
    setAiAnalyzing(null);
  };
  ...
  const analyzeAll=async()=>{
    setAiLoading(true);
    try{
      const r=await apiAiAnalyzeAll();
      if(r.results?.length){setAiResults(r.results.map(x=>({...x,ts:new Date()})));}
    }catch{}
    setAiLoading(false);
  };

  const tabs=[{id:"users",l:"👥 Utilizatori"},{id:"logs",l:"📋 Jurnal acțiuni"},{id:"suspicious",l:"🚨 Suspicioși"},{id:"permissions",l:"🔐 Permisiuni"},{id:"ai",l:"🤖 AI Monitor"}];

  return(
    <div style={{maxWidth:"100%",margin:"0 auto",padding:"22px 40px 50px"}}>
      <div className="fu" style={{marginBottom:20}}>
        <h2 style={{fontFamily:"'Lora',serif",fontSize:26,marginBottom:3,color:"#DC2626"}}>🛡️ Panou Administrare</h2>
        <p style={{color:TX2,fontSize:13}}>Gestionează utilizatori, permisiuni și monitorizează comportamentul</p>
      </div>
      <div style={{display:"flex",gap:6,marginBottom:20,borderBottom:`2px solid ${BD}`,paddingBottom:0}}>
        {tabs.map(t=><button key={t.id} className="b bg" onClick={()=>load(t.id)} style={{fontWeight:tab===t.id?800:600,color:tab===t.id?"#DC2626":TX2,background:tab===t.id?"#FEE2E2":"transparent",borderRadius:"10px 10px 0 0",borderBottom:tab===t.id?"2px solid #DC2626":"2px solid transparent",marginBottom:-2}}>{t.l}</button>)}
      </div>
      {loading&&<div style={{textAlign:"center",padding:40,color:TX2}}>Se încarcă...</div>}

      {!loading&&tab==="users"&&(
        <div className="c fu" style={{overflow:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:"#FEF2F2"}}>{["ID","Nume","Email","Rol","Suspicios","Creat","AI"].map(h=><th key={h} style={{padding:"10px 12px",textAlign:"left",fontWeight:700,color:"#991B1B",fontSize:11,textTransform:"uppercase",borderBottom:`1px solid #FECACA`}}>{h}</th>)}</tr></thead>
            <tbody>{users.map(u=><tr key={u.id} style={{borderBottom:`1px solid ${BD}`}}>
              <td style={{padding:"9px 12px",color:TX2,fontSize:12}}>{u.id}</td>
              <td style={{padding:"9px 12px",fontWeight:700}}>{u.name}</td>
              <td style={{padding:"9px 12px",color:TX2}}>{u.email}</td>
              <td style={{padding:"9px 12px"}}><span className="badge" style={{background:u.role.name==="admin"?"#FEE2E2":u.role.name==="vet"?"#DCFCE7":"#CCFBF1",color:u.role.name==="admin"?"#DC2626":u.role.name==="vet"?"#059669":T}}>{u.role.name}</span></td>
              <td style={{padding:"9px 12px"}}>{u.suspicion&&!u.suspicion.resolved?<span className="badge" style={{background:"#FEF9C3",color:"#854D0E"}} title={u.suspicion.reason}>⚠️ Suspect</span>:<span style={{color:"#22C55E",fontSize:12}}>✓</span>}</td>
              <td style={{padding:"9px 12px",color:TX2,fontSize:12}}>{new Date(u.createdAt).toLocaleDateString("ro")}</td>
              <td style={{padding:"9px 12px"}}><button className="b bg bs" disabled={aiAnalyzing===u.id} onClick={()=>analyzeUser(u.id)} style={{fontSize:11,padding:"4px 8px"}} title="Analizează cu AI">{aiAnalyzing===u.id?"⏳":"🤖"}</button></td>
            </tr>)}</tbody>
          </table>
        </div>
      )}
      ...
      {!loading&&tab==="logs"&&(
        <div className="c fu" style={{overflow:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:"#F8FAFC"}}>{["Utilizator","Rol","Acțiune","IP","Timestamp"].map(h=><th key={h} style={{padding:"9px 12px",textAlign:"left",fontWeight:700,color:TX2,fontSize:11,textTransform:"uppercase",borderBottom:`1px solid ${BD}`}}>{h}</th>)}</tr></thead>
            <tbody>{logs.map((l,i)=><tr key={l.id} style={{borderBottom:`1px solid ${BD}`,background:i%2?"#FAFBFC":"transparent"}}>
              <td style={{padding:"7px 12px",fontWeight:600}}>{l.user?.name||"?"}</td>
              <td style={{padding:"7px 12px"}}><span className="badge" style={{background:l.groupId==="admin"?"#FEE2E2":l.groupId==="vet"?"#DCFCE7":"#CCFBF1",color:l.groupId==="admin"?"#DC2626":l.groupId==="vet"?"#059669":T}}>{l.groupId}</span></td>
              <td style={{padding:"7px 12px"}}><code style={{background:"#F1F5F9",padding:"2px 6px",borderRadius:4,fontSize:11}}>{l.action}</code></td>
              <td style={{padding:"7px 12px",color:TX2,fontSize:11}}>{l.ipAddress||"—"}</td>
              <td style={{padding:"7px 12px",color:TX2,fontSize:11,whiteSpace:"nowrap"}}>{new Date(l.createdAt).toLocaleString("ro")}</td>
            </tr>)}</tbody>
          </table>
          {logs.length===0&&<p style={{textAlign:"center",color:TX2,padding:30}}>Nicio acțiune înregistrată.</p>}
        </div>
      )}

      {!loading&&tab==="suspicious"&&(
        <div style={{display:"flex",flexDirection:"column",gap:12}}>
          {susp.length===0&&<div className="c" style={{padding:40,textAlign:"center",color:TX2}}>Niciun utilizator suspicios 🎉</div>}
          {susp.map(s=>(
            <div key={s.id} className="c fu" style={{padding:18,border:`1.5px solid ${s.resolved?"#BBF7D0":"#FECACA"}`,background:s.resolved?"#F0FDF4":"#FFF5F5"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:12}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
                    <span style={{fontWeight:800,fontSize:15}}>{s.user?.name}</span>
                    <span className="badge" style={{background:"#DBEAFE",color:"#1D4ED8"}}>{s.user?.role?.name}</span>
                    {s.resolved?<span className="badge" style={{background:"#DCFCE7",color:"#166534"}}>✅ Rezolvat</span>:<span className="badge" style={{background:"#FEE2E2",color:"#991B1B"}}>🚨 Activ</span>}
                  </div>
                  <div style={{fontSize:12,color:TX2,marginBottom:4}}>{s.user?.email}</div>
                  <div style={{fontSize:13,color:"#7F1D1D",background:"#FEF2F2",padding:"6px 10px",borderRadius:8}}>⚠️ {s.reason}</div>
                  <div style={{fontSize:11,color:TX2,marginTop:6}}>Detectat: {new Date(s.flaggedAt).toLocaleString("ro")}</div>
                </div>
                {!s.resolved&&<button className="b bp bs" style={{background:"#22C55E",border:"none",whiteSpace:"nowrap"}} onClick={()=>resolve(s.user.id)}>✓ Rezolvă</button>}
              </div>
            </div>
          ))}
        </div>
      ...
      {!loading&&tab==="permissions"&&(
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:16}}>
          {perms.map(role=>(
            <div key={role.id} className="c fu" style={{padding:18}}>
              <h3 style={{fontSize:15,fontWeight:800,marginBottom:12,color:role.name==="admin"?"#DC2626":role.name==="vet"?"#059669":T}}>
                {role.name==="admin"?"🛡️":role.name==="vet"?"🏥":"🐾"} {role.name.toUpperCase()}
              </h3>
              <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
                {role.permissions.map(rp=><span key={rp.permissionId} className="badge" style={{background:"#F1F5F9",color:TX,fontSize:10}}>{rp.permission.name}</span>)}
              </div>
            </div>
          ))}
        </div>
      ...
      {!loading&&tab==="ai"&&(
        <div style={{display:"flex",flexDirection:"column",gap:16}}>
          {/* Status Ollama */}
          <div className="c fu" style={{padding:18,background:aiStatus?.running?"#F0FDF4":"#FFF5F5",border:`1.5px solid ${aiStatus?.running?"#BBF7D0":"#FECACA"}`}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
              <div>
                <div style={{fontWeight:800,fontSize:15,marginBottom:4}}>
                  {aiStatus===null?"⏳ Verificare Ollama...":aiStatus.running?"✅ Ollama activ":"❌ Ollama oprit"}
                </div>
                {aiStatus&&<div style={{fontSize:12,color:TX2}}>
                  Model activ: <code style={{background:"#F1F5F9",padding:"2px 6px",borderRadius:4}}>{aiStatus.model}</code>
                  {aiStatus.models?.length>0&&<span> | Modele disponibile: {aiStatus.models.join(", ")}</span>}
                  {!aiStatus.running&&<span style={{color:"#DC2626"}}> — Pornește Ollama și rulează <code>ollama pull phi3</code></span>}
                </div>}
              </div>
              <div style={{display:"flex",gap:8}}>
                <button className="b bg bs" onClick={()=>load("ai")} style={{fontSize:12}}>🔄 Refresh status</button>
                <button className="b bp bs" onClick={analyzeAll} disabled={aiLoading} style={{background:"#7C3AED",border:"none",fontSize:12}}>
                  {aiLoading?"⏳ Analizez...":"🤖 Analizează toți suspicioșii"}
                </button>
              </div>
            </div>
          </div>
          ...
          {aiResults.length>0&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <h3 style={{fontSize:14,fontWeight:700,color:TX2,margin:0}}>Rezultate analiză AI ({aiResults.length})</h3>
              {aiResults.map((r,i)=>(
                <div key={i} className="c fu" style={{padding:16,borderLeft:`4px solid ${r.source==="ollama"?"#7C3AED":r.source==="error"?"#DC2626":"#F59E0B"}`}}>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                    <span style={{fontWeight:800}}>{r.name}</span>
                    {r.email&&<span style={{fontSize:12,color:TX2}}>{r.email}</span>}
                    {r.role&&<span className="badge" style={{background:"#DBEAFE",color:"#1D4ED8",fontSize:10}}>{r.role}</span>}
                    <span className="badge" style={{background:r.source==="ollama"?"#EDE9FE":r.source==="error"?"#FEE2E2":"#FEF3C7",color:r.source==="ollama"?"#7C3AED":r.source==="error"?"#DC2626":"#92400E",fontSize:10,marginLeft:"auto"}}>
                      {r.source==="ollama"?"🤖 Ollama":r.source==="rules"?"📋 Reguli":"⚠️ Eroare"}
                    </span>
                  </div>
                  {r.flagReason&&<div style={{fontSize:12,color:"#7F1D1D",background:"#FEF2F2",padding:"5px 10px",borderRadius:6,marginBottom:8}}>⚠️ {r.flagReason}</div>}
                  <div style={{fontSize:13,lineHeight:1.6,color:TX,whiteSpace:"pre-wrap"}}>{r.analysis}</div>
                  <div style={{fontSize:10,color:TX2,marginTop:6}}>{r.ts?.toLocaleString?.("ro")}</div>
                </div>
              ))}
            </div>
          )}

          {/* Analiză per utilizator din lista Users */}
          <div className="c fu" style={{padding:18}}>
            <h3 style={{fontSize:14,fontWeight:700,marginBottom:12,color:TX}}>Analizează utilizator specific</h3>
            <p style={{fontSize:12,color:TX2,marginBottom:12}}>Mergi la tab-ul <strong>Utilizatori</strong> și apasă butonul 🤖 de lângă un utilizator, sau introdu ID-ul manual:</p>
            <div style={{display:"flex",gap:8}}>
              <input id="ai-uid-input" type="number" placeholder="ID utilizator..." style={{padding:"7px 12px",border:`1.5px solid ${BD}`,borderRadius:10,fontFamily:"inherit",fontSize:13,width:160}}/>
              <button className="b bp bs" style={{background:"#7C3AED",border:"none",fontSize:12}} onClick={()=>{
                const v=parseInt(document.getElementById("ai-uid-input")?.value);
                if(v>0)analyzeUser(v);
              }}>🤖 Analizează</button>
            </div>
          </div>
        </div>
...
function Chat({user,onClose}){
  ...
  useEffect(()=>{
    ...
    socket.on("system",m=>setSys(p=>[...p,m]));
  ...
  const roleColor=r=>r==="admin"?"#DC2626":r==="vet"?"#059669":T;
  const roleBg=r=>r==="admin"?"#FEE2E2":r==="vet"?"#DCFCE7":"#CCFBF1";

  return(
    <div style={{position:"fixed",bottom:20,right:20,width:360,height:500,borderRadius:20,boxShadow:"0 20px 60px rgba(0,0,0,.18)",background:"#fff",display:"flex",flexDirection:"column",zIndex:99999,overflow:"hidden",border:`1px solid ${BD}`}}>
      <div style={{background:`linear-gradient(135deg,#7C3AED,#8B5CF6)`,padding:"14px 18px",color:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontWeight:800,fontSize:15}}>💬 Chat PetVet</div><div style={{fontSize:11,opacity:.8}}>General</div></div>
        <button onClick={onClose} style={{background:"rgba(255,255,255,.2)",border:"none",borderRadius:8,color:"#fff",width:28,height:28,cursor:"pointer",fontWeight:800,fontSize:16}}>×</button>
      </div>
      <div style={{flex:1,overflowY:"auto",padding:"12px 14px",display:"flex",flexDirection:"column",gap:8,background:"#FAFBFC"}}>
        {msgs.length===0&&<div style={{textAlign:"center",color:TX2,fontSize:13,padding:20}}>Niciun mesaj încă. Fii primul! 👋</div>}
        {msgs.map((m,i)=>{
          const mine=m.senderId===user?.id;
          return(
            <div key={i} style={{display:"flex",flexDirection:"column",alignItems:mine?"flex-end":"flex-start"}}>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:2}}>
                {!mine&&<span style={{fontSize:11,fontWeight:700,color:roleColor(m.senderRole)}}>{m.senderName}</span>}
                <span className="badge" style={{background:roleBg(m.senderRole),color:roleColor(m.senderRole),fontSize:9}}>{m.senderRole}</span>
              </div>
              <div style={{maxWidth:"78%",padding:"8px 12px",borderRadius:mine?"14px 4px 14px 14px":"4px 14px 14px 14px",background:mine?`#8B5CF6`:"#fff",color:mine?"#fff":TX,fontSize:13,lineHeight:1.4,boxShadow:"0 1px 4px rgba(0,0,0,.08)"}}>
                {m.text}
              </div>
              <div style={{fontSize:10,color:TX2,marginTop:2}}>{new Date(m.timestamp).toLocaleTimeString("ro",{hour:"2-digit",minute:"2-digit"})}</div>
            </div>
          );
        })}
        {sys.slice(-3).map((s,i)=><div key={`sys${i}`} style={{textAlign:"center",fontSize:11,color:TX2,padding:"2px 0"}}>— {s} —</div>)}
        <div ref={endRef}/>
      </div>
      <div style={{padding:"10px 14px",borderTop:`1px solid ${BD}`,display:"flex",gap:8,background:"#fff"}}>
        <input value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Scrie un mesaj..." style={{flex:1,padding:"8px 12px",border:`1.5px solid ${BD}`,borderRadius:12,fontFamily:"inherit",fontSize:13,outline:"none"}}/>
        <button onClick={send} style={{background:"#8B5CF6",border:"none",borderRadius:12,color:"#fff",width:38,cursor:"pointer",fontWeight:800,fontSize:18}}>↑</button>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  const TOKEN_REFRESH_MS=25*60*1000; // refresh token la 25 min de activitate
  ...
  // Auto-logout la sesiune expirată (401 de la server)
  useEffect(()=>{
    const handler=()=>{
      CK.del("pv_session");setUser(null);setSvcs(null);setShowChat(false);setPg("login");
      toast("Sesiunea a expirat. Autentifică-te din nou.","error");
    };
    window.addEventListener("petvet:session-expired",handler);
    return()=>window.removeEventListener("petvet:session-expired",handler);
  },[]);

  // Inactivitate: auto-logout după 30 min fără activitate
  const resetInactivity=useCallback(()=>{
    if(inactivityRef.current)clearTimeout(inactivityRef.current);
    inactivityRef.current=setTimeout(()=>{
      apiLogout().catch(()=>{});CK.del("pv_session");
      setUser(null);setSvcs(null);setShowChat(false);setPg("landing");
      toast("Deconectat automat — inactivitate 30 min.","info");
    },INACTIVITY_MS);
  },[]);
  ...
  useEffect(()=>{
    ...
    resetInactivity();
    // Reîmprospătează JWT-ul la fiecare 25 min dacă utilizatorul e activ
    tokenRefreshRef.current=setInterval(()=>{
      if(getToken())apiRefreshToken().catch(()=>{});
    },TOKEN_REFRESH_MS);
    return()=>{
  ...
  const loadData=async(u)=>{
    try{
      const [p,a]=await Promise.all([apiFetchPets(u),apiFetchAppts(u)]);
      setPets(p);setAppts(a);
  ...
  const logout=async()=>{
    ...
    CK.del("pv_session");clearToken();TK.track("logout");
    setUser(null);setSvcs(null);setShowChat(false);setPg("landing");toast("Deconectat.","info");
  };
  ...
  return(
    <>
      <Toasts items={toasts}/>
      <Nav pg={pg} go={setPg} user={user} role={role} onLogout={logout} onChat={()=>setShowChat(c=>!c)}/>
      ...
      {pg==="forgot"&&<ForgotPassword go={setPg} toast={toast}/>}
      {pg==="reset"&&<ResetPassword go={setPg} onLogin={login} toast={toast}/>}
      {pg==="dashboard"&&user&&!showPicker&&<Dash pets={pets} appts={appts} go={setPg} TK={TK} role={role} svcs={activeSvcs}/>}
      {pg==="pets"&&user&&<PetsPage pets={pets} setPets={setPets} appts={appts} toast={toast} TK={TK} role={role} user={user}/>}
      ...
      {pg==="admin"&&user&&role==="admin"&&<AdminPanel/>}
      {showChat&&user&&<Chat user={user} onClose={()=>setShowChat(false)}/>}
    </>
  );
}